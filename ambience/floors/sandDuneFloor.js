class SandDuneFloor {
    ...
    createSandPattern() {
        ...
        const frequency1 = 0.015;  // Adjusted for larger area
        const frequency2 = 0.02;   // Adjusted for larger area
        const amplitude = 40;

        // Create some random "seed points" for the waves
        const seedPoints = [];
        for (let i = 0; i < 12; i++) {
            seedPoints.push({
        ...
        for (let i = 0; i < noisePixels.length; i += 4) {
            const noise = (Math.random() - 0.5) * 8; // Slightly reduced noise
